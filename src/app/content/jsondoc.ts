...
export const getCouponServices = () => {
  ...
  const resultArr = Object.entries(JSONDOC.paths).filter(([key, value]) =>
    key.includes(searchStr)
  );
  resultArr.map(([key, value]) => {
    Object.entries(value).map(([k, v]) => {
      const searchStrSchema = v['requestBody']?.content?.[
        `multipart/form-data`
      ]?.schema['$ref']
        .split('/')
        .pop();
      const schema: any = Object.entries(JSONDOC.components.schemas).find(
        ([key, value]) => key === searchStrSchema
      );
      ...
      methods.push({
        ...
        body: schema ? Object.fromEntries([schema]) : null,
  ...
  // const doStuff = v['requestBody']
  // ? Object.fromEntries(
  //     Object.entries(v['requestBody']).map(
  //       ([key, value]: [any, any]) => {
  //         const searchStrSchema: string = value?.content?.[
  //           `multipart/form-data`
  //         ]?.schema['$ref']
  //           .split('/')
  //           .pop();
  //         console.log(searchStrSchema);

  //         const schema: any = Object.entries(
  //           JSONDOC.components.schemas
  //         ).find(([key, value]) => key === searchStrSchema);
  //         return [key, schema ? Object.fromEntries(schema) : {}];
  //       }
  //     )
  //   )
  // : null;
